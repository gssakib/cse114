...
public class Homework8Methods {
	...
	public static void mobiusHelper(String x, ArrayList<String> result,
			int len, int count) {
		...
		if (count > 0) {
			...
			String proxy = String.valueOf(x.charAt(len - 1))
					+ x.substring(0, len - 1);
	...
	public static boolean bears(int initial, int goal, int increment, int n) {
		if (goal == initial) {
			...
			return true;
				...
				&& bears(initial / 2, goal, increment, n - 1)) {
			return true;
		} else if (bears(initial - increment, goal, increment, n - 1)) {
			return true;
		} else if ((initial % 2 == 0)
				&& bears(3 * (initial / 2), goal, increment, n - 1)) {
			return true;
		} else {
			return false;
		}
