	...
	public static void main(String[] args) {
		...
		for(int i =0 ; i< splited.length; i++){
			...
			for(int j = 0; j< splited[i].length(); j++){
				if((splited[i].charAt(j) >= '0' && splited[i].charAt(j)<= '9') || (splited[i].charAt(j)) == '-' ){
					count ++;
				}
			}
			//System.out.println(count);
			if (count == splited[i].length() && Integer.valueOf(splited[splited.length - 1]) < 0){
				if(Integer.valueOf(splited[i]) > 0){
					products = products * Integer.valueOf(splited[i]);
				}else if (Integer.valueOf(splited[i]) < 0 && (splited.length - 1 != i)){
