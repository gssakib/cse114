 ...
 * SBU ID - 109849940 
 * Course No. - CSE 114 
 * Final Project: Scrimish Command Line Interface
 ******ALL RIGHTS RESERVED********
 */
...
public class Scrimish {
	...
	public static void gamePlay(Cards[][] player1, Cards[][] player2,
			int starting) {
		...
		if (starting == 1) {
			// player1 starting
			while (!(player1[indexPlayer1][4].getName().equals("Destroyed"))
					&& !(player2[indexPlayer2][4].getName().equals("Destroyed"))) {
				// player 1 attacks while ai defends..
				System.out.println("");
				System.out.println("Player attacks while the AI defends");
				System.out.println("");
				...
				do {
					...
					while (!stdin.hasNextInt()) {
						stdin.next(); // Read and discard offending non-int
										// input
						System.out
								.println("Invalid Entry. Please Enter pile index number(0-4)"); // Re-prompt
						System.out.print("Choose a card from your pile: ");
					}
					...
					while (pilePlayer1 < 0 || pilePlayer1 > 4) {
						System.out
								.println("Invalid Entry. There are only 5 piles.");
						System.out
								.print("Enter index for which pile you want to attack from: ");
						pilePlayer1 = stdin.nextInt();
					}

					for (int j = 0; j < player1[pilePlayer1].length; j++) {
						if (player1[pilePlayer1][j].getName().equals(
								"Destroyed")
								&& j == 4) {
							statusDestroyed = true;
							break;
						} else if (player1[pilePlayer1][j].getName().equals(
								"Destroyed")) {
							continue;
						} else {
					...
					if (statusDestroyed == false) {
						System.out.println("The chosen card is: "
								+ player1[player1I][player1J].getName());
						...
						if (choice.equals("d")
							...
							statusOfDiscard = true;
								...
								&& (player1[player1I][player1J].getName()
										.equals("Crown"))) {
							System.out
									.println("Crown Card can't be discarded.");
						} else if (choice.equals("a")) {
							// continue with attack.
							status = false;
						} else {
							System.out
									.println("Invalid Entry. Choose option from the given choice.");
						}
					} else {

						System.out
								.println("The chosen card and the whole pile is: "
										+ player1[player1I][player1J].getName());
						System.out
								.println("Please choose another card from a different pile.");
				...
				if (statusOfDiscard == false) {
					...
					do {
						System.out
								.print("Enter index for which pile you want to attack to: ");
						...
						for (int j = 0; j < player2[pilePlayer2].length; j++) {
							if (player2[pilePlayer2][j].getName().equals(
									"Destroyed")
									&& j == 4) {
								System.out
										.println("The whole pile is destroyed. Please pick from a different pile.");
								break;
							} else if (player2[pilePlayer2][j].getName()
									.equals("Destroyed")) {
								continue;
							} else {
					...
					System.out.println("The defending card is: "
							+ player2[player2I][player2J].getName());

					// cards are printed and values compared to.
					// cards are treated according to constraints

					if (player1[player1I][player1J].getAttackVal() > player2[player2I][player2J]
							.getDefVal()
							&& !(player1[player1I][player1J].getName()
									.equals("Sheild"))
							&& !(player2[player2I][player2J].getName()
						...
						if (player2[player2I][player2J].getName().equals(
								"Crown")) {
					...
					} else if (player1[player1I][player1J].getAttackVal() < player2[player2I][player2J]
							.getDefVal()
							&& !(player1[player1I][player1J].getName()
									.equals("Sheild"))
							&& !(player2[player2I][player2J].getName()
						...
						if (player1[player1I][player1J].getName().equals(
								"Crown")) {
					...
					} else if ((player1[player1I][player1J].getName()
							.equals("Sheild"))) {
						// if shield is used to attack, it is discarded.
						player1[player1I][player1J].setName("Destroyed");
						player1[player1I][player1J].setAttackVal(-100);
						player1[player1I][player1J].setDefVal(-100);
					} else if ((player2[player2I][player2J].getName()
							.equals("Sheild"))
							&& ((player1[player1I][player1J].getName()
									.equals("Archer")) || (player1[player1I][player1J]
				...
				System.out.println("");
				System.out
						.println("Player attack ended. Now the AI attacks, while player defends");
				System.out.println("");
				...
				// continue loop until a non-crown and non-destroyed card is
				// selected from AI pile
				do {

					// AI generating i and j index of attacking card from its
					// own file.
					int AIi = (int) (Math.random() * 5);

					for (int j = 0; j < player2[AIi].length; j++) {
						if ((player2[AIi][j].getName().equals("Destroyed") && j == 4)
								|| player2[AIi][j].getName().equals("Crown")) {
							break;
						} else if (player2[AIi][j].getName()
								.equals("Destroyed")) {
							continue;
						} else {
							player2I = AIi;
				...
				System.out
						.println("(I val,J val) " + player2I + "," + player2J);

				System.out.println("The attacking Card chosen by AI is: "
						+ player2[player2I][player2J].getName());
				// continue loop until a non-destroyed card is selected from
				// player 1 pile
				status = true;
				do {

					// AI generating i and j index of defending card from player
					// 1 pile.
					int AIi = (int) (Math.random() * 5);
				...
				System.out
						.println("(I val,J val) " + player1I + "," + player1J);
				...
				if (player2[player2I][player2J].getAttackVal() > player1[player1I][player1J]
						.getDefVal()
						&& !(player1[player1I][player1J].getName()
								.equals("Sheild"))
						&& !(player2[player2I][player2J].getName()
								.equals("Sheild"))) {
					// if AI/player2 attack opponents crown card, then player 1
					// loses.
				...
				} else if (player2[player2I][player2J].getAttackVal() < player1[player1I][player1J]
						.getDefVal()
						&& !(player1[player1I][player1J].getName()
								.equals("Sheild"))
						&& !(player2[player2I][player2J].getName()
								.equals("Sheild"))) {
					// if AI/player2 attacks with its own crown card, player 1
					// wins
					...
					// if you ai attacks with its crown card player1 non-crown
					// card, then it loses
					...
					player2[player2I][player2J].setDefVal(-100);
						...
						.equals("Sheild"))
						&& ((player2[player2I][player2J].getName()
								.equals("Archer")) || (player2[player2I][player2J]
								.getName().equals("Crown")))) {
			...
			while (!(player1[indexPlayer1][4].getName().equals("Destroyed"))
				...
				System.out.println("");
				System.out.println("AI attacks while the player defends");
				System.out.println("");
				// ai attacks and player 1 defends.
				int player1I = 0;
				int player1J = 0;
				...
				System.out.println("");
				System.out
						.println("AI attack ended. Now the player 1 attacks, while AI defends");
				System.out.println("");
